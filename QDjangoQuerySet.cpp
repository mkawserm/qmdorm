...
QDjangoCompiler::QDjangoCompiler(QDjango *django, const char *modelName, const QSqlDatabase &db)
{
    this->django = django;
    driver = db.driver();
    baseModel = this->django->metaModel(modelName);
}
...
QString QDjangoCompiler::databaseColumn(const QString &name)
{
    ...
    while (bits.size() > 1) {
        ...
        if (!model.foreignFields().contains(fk)) {
            ...
            foreignModel = this->django->metaModel(fk);
            ...
            foreignModel = this->django->metaModel(model.foreignFields()[fk]);
...
QStringList QDjangoCompiler::fieldNames(bool recurse, QDjangoMetaModel *metaModel, const QString &modelPath, bool nullable)
{
    ...
    foreach (const QByteArray &fkName, metaModel->foreignFields().keys()) {
        QDjangoMetaModel metaForeign = this->django->metaModel(metaModel->foreignFields()[fkName]);
...
QString QDjangoCompiler::orderLimitSql(const QStringList &orderBy, int lowMark, int highMark)
{
    ...
    QDjangoDatabase::DatabaseType databaseType =
        QDjangoDatabase::databaseType(this->django->database());
...
QDjangoQuerySetPrivate::QDjangoQuerySetPrivate(QDjango *django, const char *modelName)
    : counter(1),
    hasResults(false),
    lowMark(0),
    highMark(0),
...
{
    QDjangoCompiler compiler(this->django,m_modelName, db);
...
QDjangoQuery QDjangoQuerySetPrivate::countQuery() const
{
    ...
    sql += limit;
    QDjangoQuery query(db);
    query.setDebug(this->django->isDebugEnabled());
    query.prepare(sql);
