...
bool QDjangoMetaField::hasDefault() const
{
    return d->default_data;
}

QString QDjangoMetaField::defaultValue() const
{
    return d->default_data_value;
}
...
*/
QDjangoMetaModel::QDjangoMetaModel(QDjango *django, const QMetaObject *meta)
    ...
    for(int i = QObject::staticMetaObject.propertyCount(); i < count; ++i)
    {
        ...
        if (infoIndex >= 0)
        {
            ...
            while (option.hasNext()) {
                ...
                else if(key == QLatin1String("default"))
                    defaultOption = value;
        ...
        field.d->null = nullOption;
        if(defaultOption.isEmpty()) field.d->default_data = false;
        else{
            field.d->default_data = true;
            field.d->default_data_value = defaultOption;
        }
        if (primaryKeyOption) {
        ...
        d->localFields << field;
    }//end of iteration over property
...
void QDjangoMetaModel::setContext(QDjango *django)
{
    this->d->django = django;
}

bool QDjangoMetaModel::hasContext() const
{
    return this->d->django!=0;
}
...
bool QDjangoMetaModel::createTable() const
{
    ...
    QDjangoQuery createQuery(this->d->django->database());
    createQuery.setDebug(this->d->django->isDebugEnabled());
...
{
    QSqlDatabase db = this->d->django->database();
    QSqlDriver *driver = db.driver();
    QDjangoDatabase::DatabaseType databaseType = this->d->django->databaseType();
    ...
    foreach (const QDjangoMetaField &field, d->localFields)
    {
        ...
        if(field.d->default_data)
            fieldSql += QLatin1String(" DEFAULT ")+field.d->default_data_value;
        if (field.d->unique)
        ...
        {
            const QDjangoMetaModel foreignMeta = this->d->django->metaModel(field.d->foreignModel);
...
bool QDjangoMetaModel::dropTable() const
{
    ...
    query.setDebug(this->d->django->isDebugEnabled());
...
QObject *QDjangoMetaModel::foreignKey(const QObject *model, const char *name) const
{
    ...
    const QDjangoMetaModel foreignMeta = this->d->django->metaModel(foreignClass);
    const QVariant foreignPk = model->property(prop + "_id");
    if (foreign->property(foreignMeta.primaryKey()) != foreignPk)
    {
        QDjangoQuerySetPrivate qs(this->d->django,foreignClass);
...
void QDjangoMetaModel::setForeignKey(QObject *model, const char *name, QObject *value) const
{
    ...
    if (value) {
        const QDjangoMetaModel foreignMeta = this->d->django->metaModel(d->foreignFields[prop]);
...
void QDjangoMetaModel::load(QObject *model, const QVariantList &properties, int &pos) const
{
    ...
    foreach (const QByteArray &fkName, d->foreignFields.keys())
    {
        ...
        if (object)
        {
            const QDjangoMetaModel foreignMeta = this->d->django->metaModel(d->foreignFields[fkName]);
...
bool QDjangoMetaModel::remove(QObject *model) const
{
    ...
    QDjangoQuerySetPrivate qs(this->d->django,model->metaObject()->className());
