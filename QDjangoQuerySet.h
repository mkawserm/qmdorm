...
//TODO foreach does not work for query
...
public:
    ...
    QDjangoQuerySet(QDjango *django);
 ...
 */
template <class T>
QDjangoQuerySet<T>::QDjangoQuerySet(QDjango *django)
{
    d = new QDjangoQuerySetPrivate(django,T::staticMetaObject.className());
    //d->django = django;
}
...
{
    T *entry = target ? target : new T(this->d->django);
...
{
    QDjangoQuerySet<T> other(this->d->django);
